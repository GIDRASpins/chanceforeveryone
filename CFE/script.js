...
function initializeInterview() {
    ...
    saveInterviewState();
    addAIMessage(interviewQuestions[currentLang].q1);
}
    ...
    switch(interviewState.stage) {
        ...
        case 3: // Вибір напрямку роботи
            if (input.includes('так') || input.includes('yes') || input.includes('да')) {
                ...
                setTimeout(() => addAIMessage(currentLang === 'ua' ? 
                    "Будь ласка, відповідь 'Так' або 'Ні'" : 
                    "Please answer 'Yes' or 'No'"), 500);
            }
            break;
        ...
        case 5.3: // Оцінка - категорія сфери
            ...
            if (matched) {
                ...
                setTimeout(() => addAIMessage(currentLang === 'ua' ?
                    "Будь ласка, виберіть одну зі запропонованих сфер" :
                    "Please choose one of the suggested fields"), 500);
            }
            break;
    }
}
